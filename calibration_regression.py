...
from utils.uncertainty_metrics import calibration_error_regression, nll
import os
...
calibration_error, predicted_pi, observed_pi = calibration_error_regression(m, s, y_val_np)
nll_score = nll(m, s, y_val_np)
total_mse = np.mean((y_val_np - m) ** 2)

figure, axes = plt.subplots(1, 1, figsize=(5, 5))

subtitle = f'{model_name}: ecr={calibration_error:.3f}, nll={nll_score:.3f}, rmse={total_mse:.3f}'
print(subtitle)
axes.plot(predicted_pi, predicted_pi, linestyle='--', color='blue', label='ideal')
axes.plot(predicted_pi, observed_pi, linestyle='-', color='orange', label=model_name)
axes.fill_between(predicted_pi.flatten(), predicted_pi.flatten(), observed_pi.flatten(), color='blue', alpha=0.20)
axes.set_xlabel('Expected CDF')
axes.set_ylabel('Observed CDF')

axes.legend()
axes.set_title(subtitle)
saving_folder = './figs/'
saving_path = os.path.join(saving_folder,'calibration error')
